...
async function getSheetsClient() {
  ...
  const id = (keys.installed || keys.web || keys).client_id;
  const secret = (keys.installed || keys.web || keys).client_secret;
  const redirect = ((keys.installed || keys.web || keys).redirect_uris || ['http://localhost'])[0];
